-- BudgetWise Database Recovery Migration
-- Ensures all required tables and structures exist without destroying existing data.
...
begin
  if not exists (
    select 1 from pg_policies where tablename = 'payment_methods' and policyname = 'Users can manage own payment_methods'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies where tablename = 'monthly_budgets' and policyname = 'Users can manage own monthly_budgets'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies where tablename = 'budget_categories' and policyname = 'Users can manage own budget_categories'
  ) then
...
-- Triggers for real-time tracking (Expense accumulation)
...
begin
  if not exists (select 1 from pg_trigger where tgname = 'on_core_transaction_insert') then
    create trigger on_core_transaction_insert after insert on public.transactions for each row execute function handle_core_transaction_insert();
  end if;
end
$$;
...
begin
  if not exists (select 1 from pg_trigger where tgname = 'on_core_transaction_delete') then
    create trigger on_core_transaction_delete after delete on public.transactions for each row execute function handle_core_transaction_delete();
  end if;
end
$$;
...
begin
  if not exists (select 1 from pg_trigger where tgname = 'on_core_transaction_update') then
    create trigger on_core_transaction_update after update on public.transactions for each row execute function handle_core_transaction_update();
  end if;
end
$$;

-- Seed default user and payment methods on signup
